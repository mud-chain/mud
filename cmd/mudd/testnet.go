...
Example:
	mudd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "mudd", "Home directory of the node's daemon configuration")
...
Example:
	mudd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("mud_%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			...
			stakingtypes.NewCommissionRates(sdk.OneDec().QuoInt64(10), sdk.OneDec().QuoInt64(5), sdk.OneDec().QuoInt64(10)),
			sdk.OneInt(),
		)
		if err != nil {
			return err
