...
var (
	inflationAddress = common.HexToAddress(types.InflationAddress)
	inflationABI     = types.MustABIJson(IInflationMetaData.ABI)
)
...
func GetMethod(name string) (abi.Method, error) {
	method := inflationABI.Methods[name]
	if method.ID == nil {
...
func GetMethodByID(input []byte) (abi.Method, error) {
	...
	for _, method := range inflationABI.Methods {
...
func GetEvent(name string) (abi.Event, error) {
	event := inflationABI.Events[name]
